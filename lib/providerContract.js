...
const { getStoredMarket, getStoredQuote, searchStoredAssets } = require("./publicMarketStore");
...
function createConnectedProvider(env = process.env) {
  ...
  return {
    ...
    async health() {
      let storedHealth = [];
      try {
        storedHealth = (await getStoredMarket()).providerStatus || [];
      } catch {
        storedHealth = [];
      }
      return [
        ...
        { service: "Database", status: "Configured", lastSuccess: storedHealth[0]?.lastSuccess || null, provider: "Supabase public REST" },
        { service: "Scheduled ingestion", status: storedHealth[1]?.status || "Not configured", lastSuccess: storedHealth[1]?.lastSuccess || null, provider: "Supabase Cron" }
      ];
    },
    async getQuote(symbol) {
      const base = await getBase(symbol);
      try {
        const stored = await getStoredQuote(base.symbol);
        if (stored?.price) {
          return normalizedQuote({
            ...stored,
            provider: stored.provider || "supabase-cache",
            dataSource: stored.dataSource || "Supabase cache",
            dataStatus: stored.dataStatus || DATA_STATUS.CACHED,
            asOf: stored.lastUpdated,
            marketStatus: stored.marketStatus || "Stored market data"
          });
        }
      } catch {
        // Live provider and demo fallbacks below keep the page functional.
      }
      try {
    ...
    async searchAssets(query) {
      ...
      let stored = [];
      try {
        stored = await searchStoredAssets(q);
      } catch {
        stored = [];
      }
      const storedMapped = stored.map((asset) => ({
        ...asset,
        provider: asset.provider || "supabase-cache",
        dataStatus: asset.dataStatus || DATA_STATUS.CACHED
      }));
      const seenStored = new Set(storedMapped.map((item) => item.symbol));
      const localMerged = [...storedMapped, ...local.filter((item) => !seenStored.has(item.symbol))];
      if (!q) return [];
      if (!stockKey) return localMerged.slice(0, 12);
      try {
        const remote = await cached(`search:${q}`, 120_000, () => alphaSearch(q, stockKey));
        const seen = new Set(localMerged.map((item) => item.symbol));
        return [...localMerged, ...remote.value.filter((item) => !seen.has(item.symbol))].slice(0, 12);
      } catch {
        return localMerged.map((item) => ({ ...item, dataStatus: item.dataStatus || DATA_STATUS.DEMO })).slice(0, 12);
      }
    },
    async search(query) {
      return this.searchAssets(query);
    ...
    async market() {
      ...
      try {
        const storedMarket = await getStoredMarket();
        if (storedMarket.assets?.length) {
          const demoSymbols = new Set(storedMarket.assets.map((item) => item.symbol));
          const missingDemo = assets.filter((item) => !demoSymbols.has(item.symbol));
          return {
            ...storedMarket,
            assets: [...storedMarket.assets, ...missingDemo],
            marketStatus: storedMarket.marketStatus || await this.getMarketStatus(),
            providerStatus: await this.health()
          };
        }
      } catch {
        // Continue to provider/demo fallback.
      }
